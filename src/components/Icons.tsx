import React from 'react';

const BaseIcon = ({ children, ...props }: React.SVGProps<SVGSVGElement>) => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
    role="img"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    {children}
  </svg>
);

export const Icons = {
  Phone: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.7 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
    </BaseIcon>
  ),
  Menu: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <line x1="4" x2="20" y1="12" y2="12" />
      <line x1="4" x2="20" y1="6" y2="6" />
      <line x1="4" x2="20" y1="18" y2="18" />
    </BaseIcon>
  ),
  X: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M18 6 6 18" />
      <path d="m6 6 12 12" />
    </BaseIcon>
  ),
  ArrowRight: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M5 12h14" />
      <path d="m12 5 7 7-7 7" />
    </BaseIcon>
  ),
  PlayCircle: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <circle cx="12" cy="12" r="10" />
      <polygon points="10 8 16 12 10 16 10 8" />
    </BaseIcon>
  ),
  Gem: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M6 3h12l4 6-10 12L2 9Z" />
      <path d="M11 3 8 9l4 12 4-12-3-6" />
      <path d="M2 9h20" />
    </BaseIcon>
  ),
  Zap: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M4 14h9l-1 9 10-11h-9l1-9z" />
    </BaseIcon>
  ),
  ShieldCheck: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z" />
      <path d="m9 12 2 2 4-4" />
    </BaseIcon>
  ),
  Calculator: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <rect width="16" height="20" x="4" y="2" rx="2" />
      <line x1="8" x2="16" y1="6" y2="6" />
      <line x1="16" x2="16" y1="14" y2="18" />
      <path d="M16 10h.01" />
      <path d="M12 10h.01" />
      <path d="M8 10h.01" />
      <path d="M12 14h.01" />
      <path d="M8 14h.01" />
      <path d="M12 18h.01" />
      <path d="M8 18h.01" />
    </BaseIcon>
  ),
  ChevronRight: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="m9 18 6-6-6-6" />
    </BaseIcon>
  ),
  ChevronLeft: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="m15 18-6-6 6-6" />
    </BaseIcon>
  ),
  StarHalf: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M12 17.77 5.82 21.02l1.18-6.88L2 9.27l6.91-1.01L12 2z" />
    </BaseIcon>
  ),
  CheckCircle2: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" />
      <path d="m9 12 2 2 4-4" />
    </BaseIcon>
  ),
  ArrowLeft: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="m12 19-7-7 7-7" />
      <path d="M19 12H5" />
    </BaseIcon>
  ),
  Send: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="m22 2-7 20-4-9-9-4Z" />
      <path d="M22 2 11 13" />
    </BaseIcon>
  ),
  Info: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <circle cx="12" cy="12" r="10" />
      <path d="M12 16v-4" />
      <path d="M12 8h.01" />
    </BaseIcon>
  ),
  ExternalLink: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M15 3h6v6" />
      <path d="M10 14 21 3" />
      <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
    </BaseIcon>
  ),
  Star: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
    </BaseIcon>
  ),
  Quote: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z" />
      <path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z" />
    </BaseIcon>
  ),
  ChevronDown: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="m6 9 6 6 6-6" />
    </BaseIcon>
  ),
  ChevronUp: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="m18 15-6-6-6 6" />
    </BaseIcon>
  ),
  MapPin: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
      <circle cx="12" cy="10" r="3" />
    </BaseIcon>
  ),
  ArrowUpRight: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M7 7h10v10" />
      <path d="M7 17 17 7" />
    </BaseIcon>
  ),
  Diamond: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M2.7 10.3a2.41 2.41 0 0 0 0 3.41l8.59 8.59a2.41 2.41 0 0 0 3.41 0l8.59-8.59a2.41 2.41 0 0 0 0-3.41L14.7 1.71a2.41 2.41 0 0 0-3.41 0Z" />
    </BaseIcon>
  ),
  Calendar: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <rect width="18" height="18" x="3" y="4" rx="2" ry="2" />
      <line x1="16" x2="16" y1="2" y2="6" />
      <line x1="8" x2="8" y1="2" y2="6" />
      <line x1="3" x2="21" y1="10" y2="10" />
    </BaseIcon>
  ),
  Mail: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <rect width="20" height="16" x="2" y="4" rx="2" />
      <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
    </BaseIcon>
  ),
  Plus: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M5 12h14" />
      <path d="M12 5v14" />
    </BaseIcon>
  ),
  Minus: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M5 12h14" />
    </BaseIcon>
  ),
  User: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" />
      <circle cx="12" cy="7" r="4" />
    </BaseIcon>
  ),
  FileText: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z" />
      <polyline points="14 2 14 8 20 8" />
      <line x1="16" x2="8" y1="13" y2="13" />
      <line x1="16" x2="8" y1="17" y2="17" />
      <line x1="10" x2="8" y1="9" y2="9" />
    </BaseIcon>
  ),
  Clock: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <circle cx="12" cy="12" r="10" />
      <polyline points="12 6 12 12 16 14" />
    </BaseIcon>
  ),
  Coins: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <circle cx="8" cy="8" r="6" />
      <path d="M18.09 10.37A6 6 0 1 1 10.34 18" />
      <path d="M7 6h1v4" />
      <path d="m16.71 13.88.7.71-2.82 2.82" />
    </BaseIcon>
  ),
  Droplet: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z" />
    </BaseIcon>
  ),
  Mold: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <circle cx="12" cy="12" r="3" />
      <path d="M12 2a5 5 0 0 0-3 9l1 1 1-1a3 3 0 0 1 3-3 3 3 0 0 1 3 3 5 5 0 0 0-3-9z" />
      <path d="M17 10l-1 1a3 3 0 0 1-3 3 3 3 0 0 1-3-3l1-1" />
    </BaseIcon>
  ),
  Thermometer: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0z" />
      <path d="M12 21a2 2 0 0 0 2-2h-4a2 2 0 0 0 2 2z" />
    </BaseIcon>
  ),
  Construction: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <rect x="2" y="6" width="20" height="8" rx="1" />
      <path d="M17 14v7" />
      <path d="M7 14v7" />
      <path d="M17 3v3" />
      <path d="M7 3v3" />
      <path d="M10 14 2.3 6.3" />
      <path d="m14 14 7.7-7.7" />
      <path d="m8 6 8 8" />
    </BaseIcon>
  ),
  Trash: (props: React.SVGProps<SVGSVGElement>) => (
    <BaseIcon {...props}>
      <path d="M3 6h18" />
      <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6" />
      <path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
      <path d="M10 11v6" />
      <path d="M14 11v6" />
    </BaseIcon>
  ),
};

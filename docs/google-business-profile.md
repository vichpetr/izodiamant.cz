...
- **Maps URL** (pro `NEXT_PUBLIC_GOOGLE_MAPS_URL` v GitHub Variables): otevřete profil v Google Maps a v adresním řádku zkopírujte celou URL (`https://www.google.com/maps/place/IZODIAMANT/@49.85...,...`). Tato URL skončí v `sameAs` v JSON-LD a v `<a>` linkách na recenze.
...
### Web – GitHub Variables (build workeru `izodiamant`, viz deployment.MD §1.1)
...
2. **Verification method**: doporučeno **HTML tag**. Token vložte do `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` (GitHub Environment `production`) a spusťte deploy – meta tag se vykreslí automaticky.
